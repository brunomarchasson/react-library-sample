...
import { Ifile, fileType, Isource } from "../../../../file-viewer-comon";
import {
  arrayBufferToUri,
  blobToUri,
  cancelRunningTask,
...
export default function useSource({
  ...
  source: Isource | null | undefined;
  error: Error | null;
} {
  const [source, setSource] = useState<Isource | null | undefined>(undefined);
  ...
  const findDocumentSource = useCallback(async (): Promise<Isource | null> => {
    if (!file) {
      return null;
    }
